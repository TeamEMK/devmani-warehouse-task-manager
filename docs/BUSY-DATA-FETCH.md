...
### 0.3 Tally Bridge — manual upload (revert, 16-Sep-2026)

15-Sep-2026 ko Drive folder se "List of Supply Outward Vouchers" auto-pick karne ki koshish ki gayi thi (naam me "supply"/"outward" ho to khud process), par Busy se ye export roz nikal kar sahi naam se Drive me daalna practically nahi ho paya — folder hamesha khali raha. **16-Sep-2026: wapas manual upload par revert** — Tally Bridge page par pehle jaisa hi **file upload** (`.xlsx` chuno) hai, turant CA-match ho kar 2W/4W InvoiceTally file ka preview + download milta hai (`/api/ops/tallyProcess`, `tally-bridge.js` `processListOfSupply`). `ops_tally_output` table aur us par bana diagnostic ("Backup schema check") hata diya gaya hai. Invoice PDF auto-fetch aur poori tarah backup-se-nikalne wala idea abhi pending hai — Busy ke DATA.ZIP backup se voucher item-lines (rate/GSTIN) ka schema pehle verify karna hoga.
